...
import { ChevronLeft, ChevronRight, ChevronsRight, Contact, MonitorCog, Shield, User, UserCog, Users, Wrench } from "lucide-react";
...
const AssignTicket = ({ ticket, fetchTicket, refreshTicket, open, setOpen, assignedOfficer = [], isDialogbox = false }) => {
    ...
    const columns = useMemo(() => [
        {
            ...
            header: ({ table }) => (
                <div className="flex items-center justify-center">
                    <Checkbox checked={table.getIsAllPageRowsSelected() || (table.getIsSomePageRowsSelected() && "indeterminate")} onCheckedChange={(value) => table.toggleAllPageRowsSelected(!!value)} disabled={[3, 4, 5, 6].includes(ticket.status.id)} />
                </div>
            ),
            cell: ({ row }) => (
                <div className="flex items-center justify-center">
                    <Checkbox checked={row.getIsSelected()} onCheckedChange={(value) => row.toggleSelected(!!value)} disabled={[3, 4, 5, 6].includes(ticket.status.id)}/>
                </div>
            )
        },
        {
    ...
    useEffect(() => {
        if(Array.isArray(ticket?.assignment) && ticket?.assignment.length > 0) {
            console.log("Assigned Officers:", ticket?.assignment);
    ...
    const handleAssign = () => {
        const isAssigned = [3, 4, 5, 6].includes(ticket.status.id); 
    ...
    return (
       <div className="flex flex-1 flex-col py-8 h-full">
            <Tabs defaultValue="all-user"  
                ...
                <div className="flex items-center justify-between px-4 lg:px-6" >
                    <div className="flex flex-row gap-2 justify-between items-center w-full">
                            ...
                            <Select defaultValue="all-user" onValueChange={(value) => setRoleFilter(value)}>
                                ...
                                <SelectContent>
                                    ...
                                    <SelectItem value="3">
                                        ...
                                        Managers</SelectItem>
                                    <SelectItem value="4">
                                        <Shield className="w-4 h-4" data-icon="inline-start"/>
                                        Officers</SelectItem>
                                </SelectContent>
                            </Select>
                        ...
                        {
                            [3, 4, 5, 6].includes(ticket.status.id) ? null : 
                            <div>
                                <Button className="text-xs" size="sm" disabled={!isNewAssigned(assignedOfficer) || table.getSelectedRowModel().rows.map(row => row.original.id).length === 0} onClick={handleAssign}>
                                    <UserCog className="w-3 h-3" data-icon="inline-start"/>
                                    Assign Officer
                                </Button>
                            </div>
                        }
                ...
                <div className="relative flex flex-col gap-4 overflow-auto px-4 lg:px-6 justify-between h-full">
                    ...
                    <div className="flex items-center justify-between px-4">
                        <div className="text-slate-700 hidden flex-1 text-sm lg:flex">
                            {table.getFilteredSelectedRowModel().rows.length} of {table.getFilteredRowModel().rows.length} users assigned.
                        </div>
